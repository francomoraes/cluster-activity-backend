...
export const joinEntity = async (
    ...
    try {
        ...
        if (!entity) {
            return res.status(404).json({ message: `${entityModel.metadata.name} not found` });
        }
        ...
        await userEntityModel.save({
            user: { id: user.id },
            [idParamName]: entity.id
        });

        return res.status(200).json({
            message: `User ${user.name} joined ${entityModel.metadata.name} ${entity.name} successfully`
        });
    } catch (error: any) {
